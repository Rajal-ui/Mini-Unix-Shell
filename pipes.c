...
int execute_pipeline(Command cmds[], int num_cmds) {
    ...
    if (num_cmds == 1) {
        ...
        if (pid == 0) {
            ...
            if (cmds[0].redir.outfile != NULL) {
                int flags = O_WRONLY | O_CREAT;
                flags |= cmds[0].redir.append ? O_APPEND : O_TRUNC;
                int fd = open(cmds[0].redir.outfile, flags, 0644);
                if (fd < 0) { perror("mysh: open"); exit(1); }
                dup2(fd, STDOUT_FILENO);
                close(fd);
            }

            if (cmds[0].redir.infile != NULL) {
    ...
    for (int i = 0; i < num_cmds; i++) {
        ...
        if (pids[i] == 0) {
            ...
            if (cmds[i].redir.outfile != NULL) {
                int flags = O_WRONLY | O_CREAT;
                flags |= cmds[i].redir.append ? O_APPEND : O_TRUNC;
                int fd = open(cmds[i].redir.outfile, flags, 0644);
                if (fd < 0) { perror("mysh: open"); exit(1); }
                dup2(fd, STDOUT_FILENO);
                close(fd);
            }

            if (cmds[i].redir.infile != NULL) {
